...
$$
X_{1}=\sin(t)+\varepsilon, \;
X_{2}=\cos(t)+\varepsilon, \;
X_{3}=\tanh(X_{1}+X_{2})+\varepsilon \\
...
$$
and plotting the trace for each projection reveals the response pattern each ridge function is targetting:
